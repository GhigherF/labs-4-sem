...
SELECT Название_Детали,Количество_заказанных 
...
SELECT Название_Детали,Количество_заказанных 
FROM ЗАКАЗЫ INNER JOIN ДЕТАЛИ
ON ЗАКАЗЫ.Артикул = ДЕТАЛИ.Артикул AND Название_Детали  LIKE '%рул%';
...
SELECT 
	Название_фирмы,
	Название_Детали,
	Количество_Заказанных,
CASE 
WHEN (Дата_Заказа > '03-20-2024') THEN 'СКОРО'
ELSE 'нескоро'
END[Скорость] ,
	Цена
FROM 
	ЗАКАЗЫ
INNER JOIN	
	ДЕТАЛИ ON ЗАКАЗЫ.Артикул = ДЕТАЛИ.Артикул
INNER JOIN
	ПОСТАВЩИКИ ON ЗАКАЗЫ.Код_Поставщика = ПОСТАВЩИКИ.Код_Поставщика
WHERE Количество_Заказанных BETWEEN 20 AND 30
ORDER BY Цена DESC;
...
SELECT 
	ISNULL(Заказы.Примечание,'***')[Звездочки] ,
	*
FROM 
	ДЕТАЛИ
